...
$validationTech = $_POST['validTech'];
$commentairesBE = htmlspecialchars($_POST['commentValiderTech']);
$dateValidationTech = date('Y-m-d');

$nbValiderBe = ajouterValiderBE($ficheId, $dateValidationTech, $validationTech, $commentairesBE);

$infosFiche = infoFiches($ficheId);

if ($validationTech == 1){
    $reqEmailOp = 'SELECT mail '
                . 'FROM utilisateur '
                . 'WHERE idTypeUtilisateur = 3';
    $resultEmailOp = Connexion::query($reqEmailOp);
    $emailOp = $resultEmailOp[0][0];

    mailAutoValidation($emailOp, $infosFiche);
} elseif ($validationTech == 0){
