...
$validationCouts = $_POST['validCouts'];
$validationDelais = $_POST['validDelais'];
$commentairesOP = htmlspecialchars($_POST['commentValiderOperationnel']);
$dateValidationCoutsDelais = date('Y-m-d');

$nbValiderBe = ajouterValiderOperationnel($ficheId, $dateValidationCoutsDelais, $validationCouts, $validationDelais, $commentairesOP);

$infosFiche = infoFiches($ficheId);

if ($validationCouts == 1 && $validationDelais == 1){
    $reqEmailQualite = 'SELECT mail '
                . 'FROM utilisateur '
                . 'WHERE idTypeUtilisateur = 4';
    $resultEmailQualite = Connexion::query($reqEmailQualite);
    $emailQualite = $resultEmailQualite[0][0];

    mailAutoValidation($emailQualite, $infosFiche);
} elseif ($validationCouts == 0 || $validationDelais == 0){
