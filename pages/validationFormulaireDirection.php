...
$validationDirection = $_POST['validDirection'];
$commentairesDire = htmlspecialchars($_POST['commentValiderDirection']);
$dateValidationDirection = date('Y-m-d');

$nbValiderBe = ajouterValiderDirection($ficheId, $dateValidationDirection, $validationDirection, $commentairesDire);

$infosFiche = infoFiches($ficheId);

if ($validationDirection == 1){
 

    mailAutoFinCircuit($infosFiche);
} elseif ($validationDirection == 0){
