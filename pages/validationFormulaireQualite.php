...
$validationQualite = $_POST['validQualite'];
$commentairesQual = htmlspecialchars($_POST['commentValiderQualite']);
$dateValidationQualite = date('Y-m-d');

$nbValiderBe = ajouterValiderSAV($ficheId, $dateValidationQualite, $validationQualite, $commentairesQual);

$infosFiche = infoFiches($ficheId);

if ($validationQualite == 1){
    if ($infosFiche[1] == 1){
        $reqEmailComm = 'SELECT mail '
                    . 'FROM utilisateur '
                    . 'WHERE idTypeUtilisateur = 8';
        $resultEmailComm = Connexion::query($reqEmailComm);
        $emailComm = $resultEmailComm[0][0];
    } elseif($infosFiche[1] == 2){
        $reqEmailComm = 'SELECT mail '
                    . 'FROM utilisateur '
                    . 'WHERE idTypeUtilisateur = 5';
        $resultEmailComm = Connexion::query($reqEmailComm);
        $emailComm = $resultEmailComm[0][0];
    }
    
    mailAutoValidation($emailComm, $infosFiche);
    
} elseif ($validationQualite == 0){
